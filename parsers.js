...
module.exports = {
  ...
  tags: (value, letKeyOnly) => {
    ...
    if (typeof (value) === "string") {
      ...
      value.split("\n").forEach((line) => {
        ...
        if (!key || (!letKeyOnly && !val)) { throw new Error("bad labels/tags format"); }
        if (val.length) { obj[key] = val.join("="); } else { obj[key] = ""; }
      });
      return obj;
    }
  ...
  objectOrFromPath: (value) => {
    if (!value) { return undefined; }
    if (typeof (value) === "string") {
      if (!fs.existsSync(value)) { throw new Error(`Couldn't find file '${value}'.`); }
