...
async function createBasicCluster(action, settings){
    ...
    return client.createBasicCluster({
        ...
        locationType: locationType || "Zonal",
        region: parsers.autocomplete(region),
        zone: parsers.autocomplete(zone),
        controlPlaneReleaseChannel: controlPlaneReleaseChannel || "REGULAR",
        version: parsers.string(version) || "1.21.5-gke.1302",
        numberOfNodes: parsers.number(numberOfNodes) || 3,
        enableAutoscaling: parsers.boolean(enableAutoscaling),
        minNode: parsers.number(minNode) || 0,
        maxNode: parsers.number(maxNode) || 3,
        machineType: parsers.autocomplete(machineType) || "e2-medium",
        nodeImage: parsers.autocomplete(nodeImage) || "COS_CONTAINERD",
        diskType: diskType || "pd-standard",
        diskSize: parsers.number(diskSize) || 10,
